...
float BMI088_ACCEL_SEN = BMI088_ACCEL_6G_SEN; //加速度计的灵敏度
float BMI088_GYRO_SEN = BMI088_GYRO_2000_SEN; //角速度灵敏度

static uint8_t res = 0; //用于存储寄存器读到的值
static uint8_t write_reg_num = 0; //用于遍历配置寄存器的计数器
static uint8_t error = BMI088_NO_ERROR; //存储错误状态
float gyroDiff[3], gNormDiff; //存储陀螺仪在校准过程中计算的误差，存储加速度计g值的变化范围
...
uint8_t BMI088_init(SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate)
{
			...
			//使用默认偏移量OFFSET
    {
        ...
        BMI088.TempWhenCali = BMI088TEMPOFFSET;
    }
