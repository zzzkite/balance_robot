...
//	  PithGyroR=-ins->Gyro[0];
	  ...
	  vmc->alpha=pi/2.0f-vmc->phi0 ;
		...
		if(vmc->first_flag==0) //第一次启动
		{
			vmc->last_phi0=vmc->phi0 ;
			vmc->first_flag=1;
		}
		//phi0和alpha的变化率刚好是相反数
...
{
	vmc->FN=vmc->F0*arm_cos_f32(vmc->theta)+vmc->Tp*arm_sin_f32(vmc->theta)/vmc->L0+10.5f;//腿部机构的力+轮子重力，这里忽略了轮子质量*驱动轮竖直方向运动加速度
